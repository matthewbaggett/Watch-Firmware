...
{
    public:
        WatchService(Debugger &debugger, EventsManager &eventsManager, AbstractRTC *rtc, WiFiManager *wifi, BluetoothManager *bt, sDOS_CPU_SCALER *cpuScaler, sDOS_FrameBuffer *frameBuffer, sDOS_TTP223 *button, sDOS_LED_MONO *led)
            : _debugger(debugger), _eventsManager(eventsManager), _rtc(rtc), _wifi(wifi), _bt(bt), _cpuScaler(cpuScaler), _frameBuffer(frameBuffer), _button(button), _led(led){};
